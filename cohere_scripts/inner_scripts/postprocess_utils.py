...
def make_image_viz(geometry, image, support, all_params):
    ...
    rampups = all_params.get('rampups', 1)
    if rampups > 1:
        # when using cohere_core dvc_utils the package needs to be specified
        dvut.set_lib_from_pkg('np')
    ...
    unwrap_phase = all_params.get('unwrap', False)
    if unwrap_phase:
        from skimage import restoration
        unwrapped_phase = restoration.unwrap_phase(np.angle(image))
        viz.add_array("imPhUW", unwrapped_phase)

    if 'imcrop' in all_params:
        mode = all_params['imcrop']
        match mode:
            case 'tight':
                arr = np.abs(image)
                imcrop_thresh = all_params['imcrop_thresh']
                buf = all_params['imcrop_margin']
            ...
            case 'fraction':
                dims = image.shape
                arrfrac = list(pad(all_params['crop_fraction'], len(dims), dims[-1]))
                viz.voi = []
