...
def _project_frames(exc: BaseException) -> list:
    """Traceback frames in the project source, outer-to-inner."""
...
def _format_frame(frame) -> str:
    """Format as ``<parent>/<basename>:<line> in <func>()``."""
...
def frame_location(exc: BaseException) -> str:
    """Deepest project frame label, or empty string."""
...
def safe_parse(field_name: str, value, *, log_error=None, log_debug=None):
    """ast.literal_eval a text-field string; on failure log and return the raw value."""
