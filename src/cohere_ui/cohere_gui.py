...
class Tabs(QTabWidget):
    ...
    def __init__(self, main_win, beamline, parent=None):
        ...
        if beamline is not None and len(beamline) > 0:
            try:
                self.beam = importlib.import_module(f'cohere_beamlines.{beamline}.beam_tabs')
            except Exception as e:
                print (e)
                msg_window(f'cannot import cohere_beamlines.{beamline} module')
                raise
...
class RecTab(QWidget):
    ...
    def init(self, tabs, main_window):
        ...
        feature_ids = ['GA', 'low resolution', 'shrink wrap', 'phase constrain', 'pcdi', 'twin', 'average', 'progress', 'live', 'global min']
        feature_dir = {'GA' : GA(),
